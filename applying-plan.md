...
When you apply a plan, the {{site.data.keyword.cloud}} resources are deployed to build a {{site.data.keyword.redhat_openshift_full}} cluster instance on your {{site.data.keyword.cloud_notm}} account with your specific choice of configuration properties. 
...
After the plan is successfully applied, it generates an `sshcommand` in the _Outputs_ section in the Terraform code. You can use this command to log in to the {{site.data.keyword.redhat_openshift_notm}} cluster. If you specified IP addresses for `ssh_allowed_ips` property only, those IP address machines would be able to use this SSH command.
...
2. Copy and paste the [Apply a plan using {{site.data.keyword.bpshort}} Python API](/docs/hpc-openshift?topic=hpc-openshift-applying-plan&interface=api#example-request-apply) example request to your Python file.
...
4. Inside the `schematics_service.apply_workspace_command` function, provide the following parameters:
  * Provide the workspace ID that you generated in the [Creating a workspace](/docs/hpc-openshift?topic=hpc-openshift-creating-workspace&interface=api) task, for example, `us-south.workspace.Terraform-Schematics-Python-Workspace.b3bbc9f5`.
